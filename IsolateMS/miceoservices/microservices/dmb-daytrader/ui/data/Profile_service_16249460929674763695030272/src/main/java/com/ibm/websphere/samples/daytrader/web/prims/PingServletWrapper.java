...
public class PingServletWrapper {
    ...
    public @ResponseBody ResponseEntity<Void> doPost(@RequestParam int callerId, @RequestParam int reqId, @RequestParam int resId) {
        try {
            PingServletServer callerObj = idObjMap.get(callerId);
    ...
    public static HashMap<Integer, PingServletServer> idObjMap = new HashMap<>();

    @RequestMapping(method = RequestMethod.GET, value = "pingservlet")
    public  @ResponseBody int PingServlet() {
        PingServletServer newServerObj = new PingServletServer();
