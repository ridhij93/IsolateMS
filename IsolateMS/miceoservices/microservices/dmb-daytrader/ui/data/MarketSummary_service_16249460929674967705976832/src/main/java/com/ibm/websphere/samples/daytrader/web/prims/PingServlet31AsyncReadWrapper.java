...
public class PingServlet31AsyncReadWrapper {
    ...
    public @ResponseBody ResponseEntity<Void> doPost(@RequestParam int callerId, @RequestParam int reqId, @RequestParam int resId) {
        try {
            PingServlet31AsyncReadServer callerObj = idObjMap.get(callerId);
    ...
    public static HashMap<Integer, PingServlet31AsyncReadServer> idObjMap = new HashMap<>();

    @RequestMapping(method = RequestMethod.GET, value = "pingservlet31asyncread")
    public  @ResponseBody int PingServlet31AsyncRead() {
        PingServlet31AsyncReadServer newServerObj = new PingServlet31AsyncReadServer();
