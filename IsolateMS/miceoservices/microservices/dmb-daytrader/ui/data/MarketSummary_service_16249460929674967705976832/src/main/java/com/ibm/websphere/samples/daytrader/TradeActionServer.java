 ...
 */
public class TradeActionServer implements TradeServices {
    ...
    public TradeActionServer(TradeServices trade) {
        if (Log.doActionTrace()) {
            Log.trace("TradeAction:TradeAction(trade)");
        }
        TradeAction.setTrade(trade);
    }

    private void createTrade() {
        if (TradeConfig.getRunTimeMode() == TradeConfig.getEJB3()) {
            try {
        ...
        } else if (TradeConfig.getRunTimeMode() == TradeConfig.getDIRECT()) {
            try {
    ...
    public QuoteDataBean getQuote(String symbol) throws Exception {
        ...
        if ((symbol == null) || (symbol.length() == 0) || (symbol.length() > 10)) {
            ...
            return new QuoteDataBean("Invalid symbol", "", 0.0, FinancialUtils.getZERO(), FinancialUtils.getZERO(), FinancialUtils.getZERO(), FinancialUtils.getZERO(), 0.0);
        }
    ...
    public static void setTradeLocal(TradeServices tradeLocal) {
        TradeActionServer.tradeLocal = tradeLocal;
    }
    ...
    public static void setTradeRemote(TradeServices tradeRemote) {
        TradeActionServer.tradeRemote = tradeRemote;
    }
