...
public class PingServlet2MDBQueueWrapper {
    ...
    public @ResponseBody ResponseEntity<Void> doPost(@RequestParam int callerId, @RequestParam int reqId, @RequestParam int resId) {
        try {
            PingServlet2MDBQueueServer callerObj = idObjMap.get(callerId);
    ...
    public static HashMap<Integer, PingServlet2MDBQueueServer> idObjMap = new HashMap<>();

    @RequestMapping(method = RequestMethod.GET, value = "pingservlet2mdbqueue")
    public  @ResponseBody int PingServlet2MDBQueue() {
        PingServlet2MDBQueueServer newServerObj = new PingServlet2MDBQueueServer();
