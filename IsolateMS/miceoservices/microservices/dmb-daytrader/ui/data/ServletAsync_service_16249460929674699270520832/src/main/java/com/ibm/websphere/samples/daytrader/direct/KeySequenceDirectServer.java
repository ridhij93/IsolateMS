...
public class KeySequenceDirectServer {
    ...
    private static Collection<?> allocNewBlock(Connection conn, String keyName, boolean inSession, boolean inGlobalTxn) throws Exception {
        try {
            ...
            stmt.setInt(1, keyVal + TradeConfig.getKEYBLOCKSIZE());
            stmt.setString(2, keyName);
            stmt.executeUpdate();
            stmt.close();
            Collection<?> block = new KeyBlock(keyVal, keyVal + TradeConfig.getKEYBLOCKSIZE() - 1);
    ...
    public static void setKeyMap(HashMap<String, Collection<?>> keyMap) {
        KeySequenceDirectServer.keyMap = keyMap;
    }
