...
public class ExplicitGCWrapper {
    ...
    public @ResponseBody ResponseEntity<Void> doPost(@RequestParam int callerId, @RequestParam int reqId, @RequestParam int resId) {
        try {
            ExplicitGCServer callerObj = idObjMap.get(callerId);
    ...
    public static HashMap<Integer, ExplicitGCServer> idObjMap = new HashMap<>();

    @RequestMapping(method = RequestMethod.GET, value = "explicitgc")
    public  @ResponseBody int ExplicitGC() {
        ExplicitGCServer newServerObj = new ExplicitGCServer();
