...
public class AdminServletServer extends HttpServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        admintype = req.getParameter(Util.getATTR_ADMINTYPE());
        Util.debug("inside AdminServlet:performTask. admintype=" + admintype);
        if ((admintype == null) || (admintype.equals(""))) {
            // Invalid Admin
            requestDispatch(getServletConfig().getServletContext(), req, resp, Util.getPAGE_ADMINHOME());
        }
        if (admintype.equals(Util.getADMIN_BACKORDER())) {
            performBackOrder(req, resp);
        } else if (admintype.equals(Util.getADMIN_SUPPLIERCFG())) {
            performSupplierConfig(req, resp);
        } else if (admintype.equals(Util.getADMIN_POPULATE())) {
    ...
    public void performSupplierConfig(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ((action == null) || (action.equals("")))
            action = Util.getACTION_GETSUPPLIER();
        Util.debug("AdminServlet.performSupplierConfig() - action=" + action);
        HttpSession session = req.getSession(true);
        if (action.equals(Util.getACTION_GETSUPPLIER())) {
            // Get supplier info
            try {
        ...
        } else if (action.equals(Util.getACTION_UPDATESUPPLIER())) {
            ...
            sendRedirect(resp, "/PlantsByWebSphere/" + Util.getPAGE_ADMINHOME());
        }
        session.setAttribute(Util.getATTR_SUPPLIER(), supplier);
        requestDispatch(getServletConfig().getServletContext(), req, resp, Util.getPAGE_SUPPLIERCFG());
    }
    ...
    public void performPopulate(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        popDB.doPopulate();
        sendRedirect(resp, "/PlantsByWebSphere/" + Util.getPAGE_HELP());
    }
    ...
    public void performBackOrder(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        action = req.getParameter(Util.getATTR_ACTION());
        if ((action == null) || (action.equals("")))
            action = Util.getACTION_GETBACKORDERS();
        Util.debug("AdminServlet.performBackOrder() - action=" + action);
        HttpSession session = req.getSession(true);
        if (action.equals(Util.getACTION_GETBACKORDERS())) {
            getBackOrders(session);
            requestDispatch(getServletConfig().getServletContext(), req, resp, Util.getPAGE_BACKADMIN());
        } else if (action.equals(Util.getACTION_UPDATESTOCK())) {
            ...
            getBackOrders(session);
            requestDispatch(getServletConfig().getServletContext(), req, resp, Util.getPAGE_BACKADMIN());
        } else if (action.equals(Util.getACTION_CANCEL())) {
            ...
            getBackOrders(session);
            requestDispatch(getServletConfig().getServletContext(), req, resp, Util.getPAGE_BACKADMIN());
        } else if (action.equals(Util.getACTION_UPDATEQUANTITY())) {
    ...
    public void getBackOrders(HttpSession session) {
        try {
            ...
            while (i.hasNext()) {
                ...
                if (!(backOrderItem.getStatus().equals(Util.getSTATUS_ADDEDSTOCK()))) {
    ...
    public void setSuppliers(SuppliersBean suppliers) {
    ...
    public void setBackOrderStock(BackOrderMgr backOrderStock) {
        this.backOrderStock = backOrderStock;
    }

    public BackOrderMgr getBackOrderStock() {
    ...
    public void setResetDB(ResetDBBean resetDB) {
        this.resetDB = resetDB;
    }

    public ResetDBBean getResetDB() {
        return resetDB;
    }
}
