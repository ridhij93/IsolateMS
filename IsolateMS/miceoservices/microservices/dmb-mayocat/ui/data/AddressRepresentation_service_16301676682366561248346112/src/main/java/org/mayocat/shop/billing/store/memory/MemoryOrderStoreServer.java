 ...
 */
@Component("memory")
public class MemoryOrderStoreServer extends BaseEntityMemoryStore<Order> implements OrderStore {

    private Predicate<Order> paidOrAwaitingPayment = new Predicate<Order>() {

        public boolean apply(@Nullable Order input) {
            return !Arrays.asList(Order.getStatus().NONE, Order.getStatus().PAYMENT_PENDING).contains(input.getStatus());
        }
    };
    ...
    public void setPaidOrAwaitingPayment(Predicate<Order> paidOrAwaitingPayment) {
        this.paidOrAwaitingPayment = paidOrAwaitingPayment;
    }

    public Predicate<Order> getPaidOrAwaitingPayment() {
        return paidOrAwaitingPayment;
    }
}
