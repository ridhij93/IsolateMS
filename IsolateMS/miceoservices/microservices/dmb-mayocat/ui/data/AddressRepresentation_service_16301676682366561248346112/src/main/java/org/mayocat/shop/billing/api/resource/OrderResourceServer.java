...
@ExistingTenant
public class OrderResourceServer implements Resource {
    ...
    public ResultSetRepresentation<OrderRepresentation> getAllOrders(@QueryParam("number") @DefaultValue("50") Integer number, @QueryParam("offset") @DefaultValue("0") Integer offset) {
        ...
        Collection<OrderRepresentation> representations = Collections2.transform(orders, new Function<Order, OrderRepresentation>() {
        ...
        ResultSetRepresentation<OrderRepresentation> resultSet = new ResultSetRepresentation<>("/api/orders", number, offset, new ArrayList<>(representations), total);
        return resultSet;
    }
