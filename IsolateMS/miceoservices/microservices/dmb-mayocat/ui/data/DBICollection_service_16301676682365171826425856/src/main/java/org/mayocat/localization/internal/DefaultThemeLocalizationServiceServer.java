 ...
 */
@Component
public class DefaultThemeLocalizationServiceServer implements ThemeLocalizationService, Initializable {
    ...
    public String getMessageTemplate(String key, Locale locale) {
        ...
        Path propertiesPath = theme.getPath().resolve(LOCALIZATION_DIRECTORY).resolve(locale.toLanguageTag() + PROPERTIES_FILE_EXTENSION);
        if (!propertiesFilesCache.containsKey(propertiesPath)) {
            ...
            switch(theme.getType()) {
                case FILE_SYSTEM:
                    ...
                    if (file.exists()) {
                        properties = Properties.getFactory().getInstance(file, Charsets.getUTF_8());
                    }
                    break;
                case CLASSPATH:
                    InputStream in = getClass().getResourceAsStream('/' + propertiesPath.toString());
                    if (in != null) {
                        properties = Properties.getFactory().getInstance(in);
                    }
                    break;
            }
    ...
    public void setPropertiesFilesCache(Map<Path, Optional<Properties>> propertiesFilesCache) {
        this.propertiesFilesCache = propertiesFilesCache;
    }

    public Map<Path, Optional<Properties>> getPropertiesFilesCache() {
        return propertiesFilesCache;
    }
}
