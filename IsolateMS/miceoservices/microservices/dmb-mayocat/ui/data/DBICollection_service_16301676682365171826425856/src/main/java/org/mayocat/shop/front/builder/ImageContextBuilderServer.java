 ...
 */
public class ImageContextBuilderServer {

    private ThemeDefinition theme;

    public ImageContextBuilderServer(ThemeDefinition theme) {
        this.theme = theme;
    }
    ...
    public ImageContext createImageContext(Image image, boolean featured) {
        ImageContext context = new ImageContext("/attachments/" + image.getAttachment().getSlug() + "." + image.getAttachment().getExtension());
        ...
        context.put("url", MessageFormat.format("/images/{0}.{1}", image.getAttachment().getSlug(), image.getAttachment().getExtension()));
        if (theme != null && theme.getImageFormats().size() > 0) {
            for (String dimensionName : theme.getImageFormats().keySet()) {
                ...
                Optional<Thumbnail> bestFit = findBestFit(image, definition.getWidth(), definition.getHeight());
                if (bestFit.isPresent()) {
                    String url = MessageFormat.format("/images/thumbnails/{0}_{1,number,#}_{2,number,#}_{3,number,#}_{4,number,#}.{5}" + "?width={6,number,#}&height={7,number,#}", image.getAttachment().getSlug(), bestFit.get().getX(), bestFit.get().getY(), bestFit.get().getWidth(), bestFit.get().getHeight(), image.getAttachment().getExtension(), definition.getWidth(), definition.getHeight());
                    context.put("theme_" + dimensionName + "_url", url);
                } else {
                    String url = MessageFormat.format("/images/{0}.{1}?width={2,number,#}&height={3,number,#}", image.getAttachment().getSlug(), image.getAttachment().getExtension(), definition.getWidth(), definition.getHeight());
                    context.put("theme_" + dimensionName + "_url", url);
                }
            }
        }
    ...
    public ImageContext createPlaceholderImageContext(boolean featured) {
        ...
        if (theme != null && theme.getImageFormats().size() > 0) {
            for (String dimensionName : theme.getImageFormats().keySet()) {
                ...
                ImageFormatDefinition definition = theme.getImageFormats().get(dimensionName);
                String url = MessageFormat.format("http://placehold.it/{0,number,#}x{1,number,#}", definition.getWidth() != null ? definition.getWidth() : definition.getHeight(), definition.getHeight() != null ? definition.getHeight() : definition.getWidth());
                context.put("theme_" + dimensionName + "_" + ContextConstants.getURL(), url);
            }
        }
        return context;
    }
    ...
    private Optional<Thumbnail> findBestFit(Image image, Integer width, Integer height) {
        if (width == null || height == null) {
            ...
            for (Thumbnail thumbnail : image.getThumbnails()) {
                if ((thumbnail.getRatio().equals("1:0") && height == null) || (thumbnail.getRatio().equals("0:1") && width == null)) {
