...
public class RequestLocalizationFilterServer implements Filter, ServletFilter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        if (isStaticPath(request.getRequestURI()) || FluentIterable.from(settings.getExcludePaths()).anyMatch(startsWithPath(request)) || webContext.getTenant() == null) {
            // Ignore static paths or paths configured as excluded
            chain.doFilter(servletRequest, servletResponse);
            return;
        }
        Optional<Cookie> languageCookie = FluentIterable.from(Arrays.asList(request.getCookies())).filter(new Predicate<Cookie>() {
    ...
    private static final boolean isLocalizedInPath(String path, List<Locale> locales) {
        List<String> fragments = ImmutableList.copyOf(Collections2.filter(Arrays.asList(path.split("/")), NOT_BLANK));
        if (fragments.size() == 0) {
            return false;
        }
    ...
    public void setSettings(LocalizationFilterSettings settings) {
        this.settings = settings;
    }

    public LocalizationFilterSettings getSettings() {
        return settings;
    }
    ...
    public static void setNOT_BLANK(Predicate<String> NOT_BLANK) {
        RequestLocalizationFilterServer.NOT_BLANK = NOT_BLANK;
    }

    public static Predicate<String> getNOT_BLANK() {
        return NOT_BLANK;
    }
}
