 ...
 */
public abstract class AbstractServiceServer<C extends AbstractSettings> extends Application<C> {
    ...
    public void initialize(Bootstrap<C> bootstrap) {
        ...
        bootstrap.getObjectMapper().configure(DeserializationFeature.getFAIL_ON_UNKNOWN_PROPERTIES(), false);
    ...
    public void run(C configuration, Environment environment) throws Exception {
        ...
        jerseyPropertiesAndFeatures.put(ResourceConfig.getPROPERTY_CONTAINER_REQUEST_FILTERS(), Joiner.on(",").join(Collections2.transform(this.requestFilters, new Function<Class, String>() {

            public String apply(Class string) {
                return string.getCanonicalName();
            }
        })));
        jerseyPropertiesAndFeatures.put(ResourceConfig.getPROPERTY_CONTAINER_RESPONSE_FILTERS(), Joiner.on(",").join(Collections2.transform(this.responseFilters, new Function<Class, String>() {
    ...
    protected void configureObjectMapper() {
        ...
        objectMapper.configure(DeserializationFeature.getFAIL_ON_UNKNOWN_PROPERTIES(), false);
    }
    ...
    private void registerServletFilters(Environment environment) throws ComponentLookupException {
        ...
        for (Map.Entry<String, ServletFilter> filter : servletFilters.entrySet()) {
            if (!Filter.class.isAssignableFrom(filter.getValue().getClass())) {
                ...
                environment.servlets().addFilter(filter.getValue().getClass().getSimpleName(), (Filter) filter.getValue()).addMappingForUrlPatterns(EnumSet.of(DispatcherType.REQUEST), true, filter.getValue().urlPattern());
            }
        }
    }
    ...
    private void registerHealthChecks(Environment environment) throws ComponentLookupException {
        ...
        for (Map.Entry<String, HealthCheck> check : healthChecks.entrySet()) {
            if (com.codahale.metrics.health.HealthCheck.class.isAssignableFrom(check.getValue().getClass())) {
                environment.healthChecks().register(check.getKey(), (com.codahale.metrics.health.HealthCheck) check.getValue());
            }
        }
    }
        ...
        for (Module module : modules.values()) {
            for (EntityMeta meta : module.getEntities()) {
                if (entities.containsKey(meta.getEntityName())) {
                    LOGGER.error("Entity with name [" + meta.getEntityName() + "] already registered. Refusing to start.");
                    System.exit(1);
                }
    ...
    private void registerJerseyClient(AbstractSettings settings, Environment environment) {
        final Client client = new JerseyClientBuilder(environment).using(settings.getJerseyClientConfiguration()).build(getName());
    ...
    public static void setStaticPaths(Set<String> staticPaths) {
        AbstractServiceServer.staticPaths = staticPaths;
    }
    ...
    public void setComponentManager(EmbeddableComponentManager componentManager) {
    ...
    public void setRequestFilters(List<Class> requestFilters) {
        this.requestFilters = requestFilters;
    }

    public List<Class> getRequestFilters() {
        return requestFilters;
    }

    public void setResponseFilters(List<Class> responseFilters) {
        this.responseFilters = responseFilters;
    }

    public List<Class> getResponseFilters() {
