 ...
 */
public abstract class AbstractScopeCookieContainerFilterServer<T extends WebScope> implements ContainerResponseFilter, ContainerRequestFilter {
    ...
    protected void deleteCookie(ContainerResponse containerResponse) {
        ...
        containerResponse.getHttpHeaders().add("Set-Cookie", getScopeAndCookieName() + "=deleted;Path=/;Expires=Thu, 01-Jan-1970 00:00:01 GMT");
    }
    ...
    protected void storeScopeInCookies(WebContext context, ContainerResponse containerResponse) {
        ...
        try {
            ...
            NewCookie scope = new NewCookie(getScopeAndCookieName(), cookieValue, "/", null, 1, "", getCookieDuration(), false);
            Response cookieResponse = Response.fromResponse(containerResponse.getResponse()).cookie(scope).build();
    ...
    private String computeSignature(String message) throws GeneralSecurityException {
        ...
        SecretKeySpec signingKey = new SecretKeySpec(securitySettings.getSigningKey().getBytes(), HMAC_SHA256_ALGORITHM);
        Mac mac = Mac.getInstance(HMAC_SHA256_ALGORITHM);
        mac.init(signingKey);
        byte[] rawHmac = mac.doFinal(message.getBytes(Charsets.getUTF_8()));
    ...
    public static String getHMAC_SHA256_ALGORITHM() {
