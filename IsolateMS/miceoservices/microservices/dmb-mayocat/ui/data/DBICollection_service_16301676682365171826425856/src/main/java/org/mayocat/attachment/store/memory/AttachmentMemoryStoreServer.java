 ...
 */
@Component("memory")
public class AttachmentMemoryStoreServer extends BaseEntityMemoryStore<Attachment> implements AttachmentStore {

    private static final Function<Attachment, LoadedAttachment> CAST_AS_LOADED_ATTACHMENT = new Function<Attachment, LoadedAttachment>() {
    ...
    private Predicate<Attachment> withExtension(final String... extensions) {
        return new Predicate<Attachment>() {
            ...
            public boolean apply(@Nullable Attachment input) {
                return input.getSlug().equals(input.getSlug()) && Arrays.asList(extensions).contains(input.getExtension());
            }
        };
    }
    ...
    public LoadedAttachment findAndLoadBySlug(final String slug) {
        return FluentIterable.from(entities.values()).filter(withSlug(slug)).transform(CAST_AS_LOADED_ATTACHMENT).limit(1).first().orNull();
    }

    @Override
    public LoadedAttachment findAndLoadBySlugAndExtension(final String fileName, final String extension) {
        return FluentIterable.from(entities.values()).transform(CAST_AS_LOADED_ATTACHMENT).filter(withExtension(extension)).limit(1).first().orNull();
    }
    ...
    public List<Attachment> findAllChildrenOf(Entity parent, List<String> extensions) {
        return FluentIterable.from(entities.values()).filter(withParent(parent)).filter(withExtension(extensions.toArray(new String[extensions.size()]))).toList();
    }

    @Override
    public List<Attachment> findAllChildrenOfParentIds(List<UUID> parents) {
        return FluentIterable.from(entities.values()).filter(withParentId(parents.toArray(new UUID[parents.size()]))).toList();
    }

    @Override
    public List<Attachment> findAllChildrenOfParentIds(List<UUID> parents, List<String> extensions) {
        return FluentIterable.from(entities.values()).filter(withParentId(parents.toArray(new UUID[parents.size()]))).filter(withExtension(extensions.toArray(new String[extensions.size()]))).toList();
    }
    ...
    public static Function<Attachment, LoadedAttachment> getCAST_AS_LOADED_ATTACHMENT() {
        return CAST_AS_LOADED_ATTACHMENT;
    }
}
