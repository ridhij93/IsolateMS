...
public class DBICollectionStoreServer extends DBIEntityStore implements CollectionStore, Initializable {
    ...
    public void moveCollection(String collectionToMove, String collectionToMoveRelativeTo, RelativePosition relativePosition) throws InvalidMoveOperation {
        ...
        MoveEntityInListOperation<Collection> moveOp = new MoveEntityInListOperation<Collection>(allCollections, collectionToMove, collectionToMoveRelativeTo, relativePosition);
        if (moveOp.hasMoved()) {
    ...
    public void moveProductInCollection(Collection collection, String productToMove, String productToMoveRelativeTo, RelativePosition relativePosition) throws InvalidMoveOperation {
        ...
        MoveEntityInListOperation<Product> moveOp = new MoveEntityInListOperation<>(categoryProducts, productToMove, productToMoveRelativeTo, relativePosition);
        if (moveOp.hasMoved()) {
    ...
    public void updateCollectionTree(List<PositionedEntity<Collection>> positionedCollections) {
        this.dao.begin();
        List<Collection> collections = FluentIterable.from(positionedCollections).transform(new Function<PositionedEntity<Collection>, Collection>() {

            public Collection apply(PositionedEntity<Collection> input) {
                return input.getEntity();
            }
        }).toList();
        List<Integer> positions = FluentIterable.from(positionedCollections).transform(new Function<PositionedEntity<Collection>, Integer>() {
    ...
    public List<Collection> findAll(Integer number, Integer offset) {
        return AddonsHelper.withAddons(this.dao.findAll(COLLECTION_TABLE_NAME, COLLECTION_POSITION, getTenant(), number, offset), dao);
    }
    ...
    public static String getCOLLECTION_TABLE_NAME() {
        return COLLECTION_TABLE_NAME;
    }

    public static String getCOLLECTION_POSITION() {
