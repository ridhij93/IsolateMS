...
@ExistingTenant
public class CheckoutResourceServer implements Resource {
    ...
    public Object checkout(final MultivaluedMap data) {
        ...
        if (webContext.getUser() == null && !checkoutSettings.isGuestCheckoutEnabled().getValue()) {
            return Response.status(Response.Status.FORBIDDEN).entity("Configuration does not allow guest checkout").build();
        }
        ...
        if (checkoutRequest.getErrors().size() != 0) {
            ...
            return new WebView().template("checkout/form.html").with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
        }
        try {
    ...
    private Object generateCheckoutResponse(CheckoutResponse response) throws URISyntaxException {
        ...
        switch(paymentRequest.getNextAction()) {
            ...
            case INTERNAL_FORM:
                bindings.put("formURL", paymentRequest.getRedirectionTarget().isPresent() ? paymentRequest.getRedirectionTarget().get() : "/checkout/" + response.getOrder().getId() + "/payment");
                ...
                return new WebView().template("checkout/payment.html").with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
            case MANUAL_VALIDATION:
            case NONE:
            default:
                bindings.putAll(prepareContext(response.getOrder(), response.getOrder().getCustomer(), Optional.fromNullable(response.getOrder().getBillingAddress()), Optional.fromNullable(response.getOrder().getDeliveryAddress()), webContext.getTenant(), configurationService.getSettings(GeneralSettings.class).getLocales().getMainLocale().getValue()));
                String template = "checkout/success.html";
                bindings.put("checkoutResult", "success");
                if (paymentRequest.getNextAction().equals(RequiredAction.getMANUAL_VALIDATION())) {
                    bindings.put("checkoutResult", "paymentPending");
                    template = "checkout/pending.html";
                } else if (paymentRequest.getStatus().equals(PaymentStatus.getFAILED()) || paymentRequest.getStatus().equals(PaymentStatus.getREFUSED())) {
                    bindings.put("checkoutResult", "failed");
                    template = "checkout/failed.html";
                }
                return new WebView().template(template).with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
        }
    }
    ...
    public Object internalPayment(@PathParam("orderId") UUID orderId, MultivaluedMap<String, String> data) {
        try {
            ...
            paymentData.put(BasePaymentData.getCURRENCY(), order.getCurrency());
            paymentData.put(BasePaymentData.getORDER_ID(), order.getId());
            paymentData.put(BasePaymentData.getCUSTOMER(), order.getCustomer());
            if (order.getBillingAddress() != null) {
                paymentData.put(BasePaymentData.getBILLING_ADDRESS(), order.getBillingAddress());
            }
            paymentData.put(BasePaymentData.getDELIVERY_ADDRESS(), order.getDeliveryAddress());
            paymentData.put(BasePaymentData.getORDER(), order);
            if (data.containsKey("cardNumber")) {
                paymentData.put(CreditCardPaymentData.getCARD_NUMBER(), data.getFirst("cardNumber"));
                paymentData.put(CreditCardPaymentData.getHOLDER_NAME(), data.getFirst("holderName"));
                paymentData.put(CreditCardPaymentData.getEXPIRATION_MONTH(), data.getFirst("expiryMonth"));
                paymentData.put(CreditCardPaymentData.getEXPIRATION_YEAR(), data.getFirst("expiryYear"));
                paymentData.put(CreditCardPaymentData.getVERIFICATION_CODE(), data.getFirst("cvv"));
            }
    ...
    public WebView returnFromPaymentService(@Context UriInfo uriInfo, @PathParam("order") String orderId) {
        ...
        if (StringUtils.isNotBlank(orderId)) {
            ...
            if (order != null) {
                Optional<Address> da = order.getDeliveryAddress() != null ? Optional.fromNullable(order.getDeliveryAddress()) : Optional.<Address>absent();
                Optional<Address> ba = order.getBillingAddress() != null ? Optional.fromNullable(order.getBillingAddress()) : Optional.<Address>absent();
                bindings.putAll(prepareContext(order, order.getCustomer(), ba, da, webContext.getTenant(), configurationService.getSettings(GeneralSettings.class).getLocales().getMainLocale().getValue()));
            }
        }
        return new WebView().template("checkout/success.html").with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
    }
    ...
    public WebView cancelFromPaymentService(@PathParam("orderId") UUID orderId) {
        try {
            ...
            return new WebView().template("checkout/exception.html").with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
        }
        Map<String, Object> bindings = new HashMap<>();
        return new WebView().template("checkout/cancelled.html").with(WebView.getOption().FALLBACK_ON_DEFAULT_THEME).data(bindings);
    }
    ...
    private Map<String, Object> prepareContext(Order order, Customer customer, Optional<Address> ba, Optional<Address> da, Tenant tenant, Locale locale) {
        ...
        MoneyFormatter formatter = new MoneyFormatterBuilder().appendAmount(MoneyAmountStyle.of(locale)).appendLiteral(" ").appendCurrencySymbolLocalized().toFormatter();
        CurrencyUnit currencyUnit = CurrencyUnit.of(order.getCurrency());
        String grandTotal = formatter.withLocale(locale).print(Money.of(currencyUnit, order.getGrandTotal(), RoundingMode.HALF_EVEN));
        String itemsTotal = formatter.withLocale(locale).print(Money.of(currencyUnit, order.getItemsTotal(), RoundingMode.HALF_EVEN));
        ...
        for (OrderItem item : items) {
            ...
            orderItem.put("unitPrice", formatter.withLocale(locale).print(Money.of(currencyUnit, unitPrice, RoundingMode.HALF_EVEN)));
            orderItem.put("itemTotal", formatter.withLocale(locale).print(Money.of(currencyUnit, itemTotal, RoundingMode.HALF_EVEN)));
            orderItems.add(orderItem);
        }
        context.put("items", orderItems);
        if (order.getShipping() != null) {
            String shippingTotal = formatter.withLocale(locale).print(Money.of(currencyUnit, order.getShipping(), RoundingMode.HALF_EVEN));
    ...
    public static String getPAYMENT_RETURN_PATH() {
        return PAYMENT_RETURN_PATH;
    }

    public static String getPAYMENT_CANCEL_PATH() {
        return PAYMENT_CANCEL_PATH;
    }
    ...
    public void setCheckoutRegister(CheckoutRegister checkoutRegister) {
        this.checkoutRegister = checkoutRegister;
    }

    public CheckoutRegister getCheckoutRegister() {
