...
public class AttachmentDataLoaderServer implements DataLoaderAssistant {
    ...
    public <E extends Entity> void loadList(List<EntityData<E>> entities, LoadingOption... options) {
        ...
        if (optionsAsList.contains(AttachmentLoadingOptions.FEATURED_IMAGE_ONLY)) {
            ...
            List<UUID> imageIds = FluentIterable.from(entities).transform(new Function<EntityData<E>, UUID>() {
                ...
                public UUID apply(EntityData<E> input) {
                    if (!HasFeaturedImage.class.isAssignableFrom(input.getEntity().getClass())) {
                        throw new RuntimeException("Failed to load attachment list with featured image only option : entity does not implement HasFeaturedImage");
                    }
                    return ((HasFeaturedImage) input.getEntity()).getFeaturedImageId();
                }
            }).filter(Predicates.notNull()).toList();
            attachments = imageIds.size() > 0 ? this.attachmentStore.findByIds(imageIds) : Collections.<Attachment>emptyList();
        } else {
            ...
            attachments = ids.size() > 0 ? this.attachmentStore.findAllChildrenOfParentIds(ids) : Collections.<Attachment>emptyList();
        }
        for (final EntityData entity : entities) {
            List<Attachment> thisEntityAttachments = FluentIterable.from(attachments).filter(new Predicate<Attachment>() {
