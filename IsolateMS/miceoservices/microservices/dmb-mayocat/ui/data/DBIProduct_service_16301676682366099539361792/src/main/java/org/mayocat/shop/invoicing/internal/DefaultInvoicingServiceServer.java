 ...
 */
@Component
public class DefaultInvoicingServiceServer implements InvoicingService, Initializable {
    ...
    public void generatePdfInvoice(Order order, OutputStream outputStream) throws InvoicingException {
        ...
        try {
            pdfTemplateRenderer.generatePDF(outputStream, invoicesFolderPath.get().resolve(INVOICE_TEMPLATE_FILE_NAME), invoicesFolderPath.get(), invoiceContext);
    ...
    protected Map<String, Object> prepareInvoiceContext(final Order order) throws InvoicingException {
        ...
        orderWebObject.setItems(FluentIterable.from(order.getOrderItems()).transform(new Function<OrderItem, OrderItemWebObject>() {
    ...
    private Optional<Path> getInvoicingDirectory(Tenant tenant) {
        if (tenant != null) {
            Path localInvoicingDirectory = fileManager.resolvePermanentFilePath(Paths.get("tenants").resolve(tenant.getSlug()).resolve("invoicing"));
    ...
    public void setPdfTemplateRenderer(PdfTemplateRenderer pdfTemplateRenderer) {
        this.pdfTemplateRenderer = pdfTemplateRenderer;
    }

    public PdfTemplateRenderer getPdfTemplateRenderer() {
    ...
    public void setDao(InvoiceNumberDao dao) {
        this.dao = dao;
    }

    public InvoiceNumberDao getDao() {
        return dao;
    }

    public static String getINVOICE_TEMPLATE_FILE_NAME() {
        return INVOICE_TEMPLATE_FILE_NAME;
    }
}
