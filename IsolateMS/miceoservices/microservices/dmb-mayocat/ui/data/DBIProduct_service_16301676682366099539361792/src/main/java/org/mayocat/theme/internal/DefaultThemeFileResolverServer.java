 ...
 */
@Component
public class DefaultThemeFileResolverServer implements ThemeFileResolver {
    ...
    public Template getGlobalTemplate(String name, Optional<Breakpoint> breakpoint) throws TemplateNotFoundException {
        try {
            return new Template(generateTemplateId(name, breakpoint), Resources.toString(Resources.getResource("templates/" + name), Charsets.getUTF_8()), true);
        } catch (IOException e) {
    ...
    private String generateTemplateId(String layoutName, Optional<Breakpoint> breakpoint) {
        ...
        String templateId = themeName.length() + themeName + "_" + breakpoint.toString().length() + breakpoint.toString() + "_" + layoutName.length() + layoutName;
        ...
        switch(theme.getType()) {
            case FILE_SYSTEM:
                if (path.toFile().exists()) {
                    return new ThemeResource(ThemeResource.getType().FILE, path);
                }
                break;
            case CLASSPATH:
                try {
                    Resources.getResource(path.toString());
                    return new ThemeResource(ThemeResource.getType().CLASSPATH_RESOURCE, path);
    ...
    private String getTemplateContent(Theme theme, String name, Optional<Breakpoint> breakpoint) throws IOException {
        ...
        switch(resource.getType()) {
            default:
            case FILE:
                return Files.toString(resource.getPath().toFile(), Charsets.getUTF_8());
            case CLASSPATH_RESOURCE:
                return Resources.toString(Resources.getResource(resource.getPath().toString()), Charsets.getUTF_8());
        }
    }
    ...
    public static String getINDEX_HTML() {
        return INDEX_HTML;
    }
