 ...
 */
public abstract class AbstractPositionedEntityMemoryStoreServer<T extends Identifiable, P extends PositionedEntity<T>> implements Store<T, UUID> {
    ...
    protected Function<P, T> positionedToEntity = new Function<P, T>() {
        ...
        @Nullable
        public T apply(@Nullable P input) {
    ...
    protected List<P> allPositioned() {
        return FluentIterable.from(Ordering.natural().onResultOf(positionOrdering).sortedCopy(FluentIterable.from(store.findAll(0, 0)))).toList();
    }

    protected List<T> all() {
        return FluentIterable.from(Ordering.natural().onResultOf(positionOrdering).sortedCopy(FluentIterable.from(store.findAll(0, 0)))).transform(positionedToEntity).toList();
    }
    ...
    private Integer nextPosition() {
        List<Integer> positions = FluentIterable.from(this.store.findAll(0, 0)).transform(new Function<PositionedEntity, Integer>() {

            @Nullable
            public Integer apply(@Nullable PositionedEntity input) {
                return input.getPosition();
            }
        }).toList();
    ...
    public void setPositionedToEntity(Function<P, T> positionedToEntity) {
        this.positionedToEntity = positionedToEntity;
    }

    public Function<P, T> getPositionedToEntity() {
        return positionedToEntity;
    }
    ...
    public void setPositionOrdering(Function<P, Integer> positionOrdering) {
        this.positionOrdering = positionOrdering;
    }

    public Function<P, Integer> getPositionOrdering() {
        return positionOrdering;
    }

    public void setStore(BaseEntityMemoryStore<P> store) {
        this.store = store;
    }

    public BaseEntityMemoryStore<P> getStore() {
        return store;
    }
}
