...
@Component
@Named("handlebars")
public class HandlebarsEngineServer extends AbstractRhinoEngine implements TemplateEngine {
    ...
    public HandlebarsEngineServer() throws IOException {
    ...
    protected void initializeEngine() {
        try {
            ...
            try {
                ...
                engineContext.evaluateReader(globalScope, helpersReader, JSFile.MAYO_HELPERS.getFileName(), 0, null);
                ...
                engineContext.evaluateReader(globalScope, swagHelpersReader, JSFile.SWAG_HELPERS.getFileName(), 0, null);
                // All other helpers declared as handlers.
                for (String scriptName : helpers.keySet()) {
                    Reader helper = new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream("/" + helpers.get(scriptName).getPath().toString())));
    ...
    public synchronized void register(Template template) throws TemplateEngineException {
        ...
        try {
            ...
            try {
                context.evaluateString(compileScope, "templates[name] = Handlebars.compile(content, {noEscape: true});", "JHBSCompiler", 0, null);
                if (template.isPartial()) {
                    context.evaluateString(compileScope, "Handlebars.registerPartial(name, templates[name]);", "JHBSPartialRegister", 0, null);
                }
    ...
    public synchronized String render(String templateName, String json) throws TemplateEngineException {
        ...
        try {
            ...
            try {
                context.evaluateString(renderScope, "writer.write( templates[name](JSON.parse(json)) )", "JHBSRenderer", 0, null);
    ...
    private static Reader getResourceReader(String resource) throws IOException {
        return Resources.newReaderSupplier(Resources.getResource(resource), Charsets.getUTF_8()).getInput();
    }
    ...
    public void setHelpers(Map<String, HelpersScript> helpers) {
        this.helpers = helpers;
    }

    public Map<String, HelpersScript> getHelpers() {
        return helpers;
    }
}
