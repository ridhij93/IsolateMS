...
public class RequestContextInitializerServer implements ServletRequestListener, EventListener {
    ...
    @Inject
    // -> This is the ThreadLocalWebContext
    @Named("default")
    ...
    private static final Predicate<String> IS_BLANK = matchesAllOf(CharMatcher.getWHITESPACE());
    ...
    public void requestInitialized(ServletRequestEvent servletRequestEvent) {
        ...
        List<Locale> alternativeLocales = FluentIterable.from(localesSettings.getOtherLocales().getValue()).filter(Predicates.notNull()).toList();
        String canonicalPath = path;
        if (!alternativeLocales.isEmpty()) {
            for (Locale locale : alternativeLocales) {
                List<String> fragments = ImmutableList.copyOf(Collections2.filter(Arrays.asList(path.split("/")), Predicates.not(IS_NULL_OR_BLANK)));
        ...
        requestBuilder.baseURI(getBaseURI(servletRequestEvent)).canonicalPath(canonicalPath).path(path).breakpoint(breakpoint);
    ...
    private URI getBaseURI(ServletRequestEvent event) {
        ...
        if ((request.getServerPort() == 80) || (request.getServerPort() == 443)) {
            return URI.create(request.getScheme() + "://" + request.getServerName() + request.getContextPath() + '/');
        } else {
            return URI.create(request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + '/');
        }
    }
    ...
    public void setTenantResolver(Provider<TenantResolver> tenantResolver) {
        this.tenantResolver = tenantResolver;
    }

    public Provider<TenantResolver> getTenantResolver() {
    ...
    public void setBreakpointDetector(UserAgentBreakpointDetector breakpointDetector) {
        this.breakpointDetector = breakpointDetector;
    }

    public UserAgentBreakpointDetector getBreakpointDetector() {
    ...
    public static Predicate<String> getIS_BLANK() {
        return IS_BLANK;
    }

    public static Predicate<String> getIS_NULL_OR_BLANK() {
