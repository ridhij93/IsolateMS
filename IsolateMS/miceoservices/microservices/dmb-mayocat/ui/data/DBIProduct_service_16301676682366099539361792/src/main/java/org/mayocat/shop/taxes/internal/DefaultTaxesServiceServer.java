 ...
 */
@Component
public class DefaultTaxesServiceServer implements TaxesService {
    ...
    public PriceWithTaxes getPriceWithTaxes(Taxable taxable) {
        if (!taxable.getActualUnitPrice().isPresent()) {
            return new PriceWithTaxes(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
        }
        BigDecimal itemVatRate = getVatRate(taxable);
        BigDecimal unitPrice = taxable.getActualUnitPrice().get();
        return new PriceWithTaxes(unitPrice.multiply(BigDecimal.ONE.add(itemVatRate)), unitPrice, unitPrice.multiply(itemVatRate));
    }
