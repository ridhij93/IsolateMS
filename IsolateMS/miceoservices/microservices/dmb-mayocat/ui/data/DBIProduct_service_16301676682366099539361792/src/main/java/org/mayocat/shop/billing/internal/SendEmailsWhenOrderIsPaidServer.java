...
public class SendEmailsWhenOrderIsPaidServer implements EventListener {
    ...
    private MailTemplate getCustomerNotificationEmail(Tenant tenant, String customerEmail, Locale locale) {
        return new MailTemplate().from(getTenantContactEmail(tenant)).to(customerEmail).template("order-paid-customer").locale(locale);
    }
    ...
    private MailTemplate getTenantNotificationEmail(Tenant tenant, Locale locale) {
        return new MailTemplate().from(generalSettings.getNotificationsEmail()).to(getTenantContactEmail(tenant)).template("order-paid-tenant").locale(locale);
    }
    ...
    private String getTenantContactEmail(Tenant tenant) {
        if (tenant == null) {
            ...
            return StringUtils.isNotBlank(tenant.getContactEmail()) ? tenant.getContactEmail() : generalSettings.getNotificationsEmail();
        }
    }
    ...
    private Map<String, Object> prepareMailContext(Order order, Customer customer, Optional<Address> ba, Optional<Address> da, Tenant tenant, Locale locale) {
        ...
        for (OrderItem item : items) {
            ...
            itemData.put("unitPrice", formatter.withLocale(locale).print(Money.of(currencyUnit, item.getUnitPrice(), RoundingMode.HALF_EVEN)));
            itemData.put("itemTotal", formatter.withLocale(locale).print(Money.of(currencyUnit, item.getItemTotal(), RoundingMode.HALF_EVEN)));
