 ...
 */
@Component("memory")
public class MemoryProductStoreServer extends AbstractPositionedEntityMemoryStore<Product, DefaultPositionedEntity<Product>> implements ProductStore {
    ...
    private static final Predicate<Product> withTitleLike(final String title) {
        return new Predicate<Product>() {
            ...
            public boolean apply(@Nullable Product input) {
                return StringUtils.stripAccents(input.getTitle()).toLowerCase().indexOf(StringUtils.stripAccents(title).toLowerCase()) >= 0;
            }
        };
    }
    ...
    public List<Product> findOnShelfForCollection(final Collection collection, Integer number, Integer offset) {
        return FluentIterable.from(all()).filter(new Predicate<Product>() {
            ...
            public boolean apply(@Nullable Product input) {
                return input.getCollections().isLoaded() && input.getCollections().get().contains(collection) && input.getOnShelf();
            }
        }).toList();
    }
    ...
    public List<Product> findAllOnShelf(Integer number, Integer offset) {
        ...
        return FluentIterable.from(all()).filter(ON_SHELF).skip(offset).limit(number).toList();
    }
    ...
    public List<Product> findAllWithTitleLike(String title, Integer number, Integer offset) {
        ...
        return FluentIterable.from(all()).filter(withTitleLike(title)).skip(offset).limit(number).toList();
    }
    ...
    public static Predicate<Product> getON_SHELF() {
        return ON_SHELF;
    }

    public static Predicate<Product> getNOT_A_VARIANT() {
