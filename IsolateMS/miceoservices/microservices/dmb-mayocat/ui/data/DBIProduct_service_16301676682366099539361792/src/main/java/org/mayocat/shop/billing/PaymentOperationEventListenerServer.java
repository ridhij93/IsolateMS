...
public class PaymentOperationEventListenerServer implements EventListener {
    ...
    public void onEvent(Event event, Object source, Object data) {
        ...
        if (operation.getResult().equals(PaymentOperation.getResult().CAPTURED)) {
            Order order = orderStore.get().findById(operation.getOrderId());
            this.logger.debug("Updating order paid status");
            try {
                order.setStatus(Order.getStatus().PAID);
        ...
        if (operation.getResult().equals(PaymentOperation.getResult().FAILED)) {
            try {
                ObjectMapper mapper = new ObjectMapper();
                String body = MessageFormat.format("A payment failed for gateway {0} and order ID {1} : \n\n{2}", operation.getGatewayId(), operation.getOrderId(), mapper.writeValueAsString(operation.getMemo()));
                Mail mail = new Mail().to(siteSettings.getAdministratorEmail()).from(siteSettings.getContactEmail()).subject("WARNING: Payment failed").text(body);
