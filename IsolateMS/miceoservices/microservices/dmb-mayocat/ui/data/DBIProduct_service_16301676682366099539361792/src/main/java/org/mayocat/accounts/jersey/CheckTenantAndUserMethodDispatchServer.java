...
public class CheckTenantAndUserMethodDispatchServer implements ResourceMethodDispatchAdapter, Provider {
    ...
    private class CheckValidTenantRequestDispatcher implements RequestDispatcher {
        ...
        public void dispatch(Object resource, HttpContext httpContext) {
            if (context.getTenant() == null) {
                throw new WebApplicationException(ErrorUtil.buildError(Response.Status.NOT_FOUND, StandardError.getNOT_A_VALID_TENANT(), "No valid tenant at this address"));
            }
    ...
    private class CheckAuthorizationMethodDispatcher implements RequestDispatcher {
        ...
        public void dispatch(Object resource, HttpContext httpContext) {
            ...
            if (user != null) {
                if (!this.checkAuthorization(user)) {
                    throw new WebApplicationException(Response.status(Response.Status.FORBIDDEN).entity(new Error(Response.Status.FORBIDDEN, StandardError.getINSUFFICIENT_PRIVILEGES(), "Insufficient privileges")).type(MediaType.APPLICATION_JSON).build());
                }
            } else {
                if (this.isTenantEmptyOfUser() && this.isCreateUserResource()) {
                    ...
                    throw new WebApplicationException(Response.status(Response.Status.UNAUTHORIZED).header("WWW-Authenticate", "realm=\"Mayocat Shop\"").entity("Request is not properly authenticated.").type(MediaType.TEXT_PLAIN_TYPE).build());
                }
            }
            underlying.dispatch(resource, httpContext);
        }
        ...
        private boolean checkAuthorization(User user) {
            ...
            if (context.getRequest().isApiRequest() && !gatekeeper.userHasRole(user, Role.getADMIN())) {
        ...
        private boolean isCreateUserResource() {
            return TenantUserApi.class.isAssignableFrom(this.method.getDeclaringResource().getResourceClass()) && this.method.getHttpMethod().equals(HttpMethod.POST) && this.method.getResource().getPath().getValue().endsWith("/api/user");
        }
    }
    ...
    private class AnnotationResourceMethodDispatchProvider implements ResourceMethodDispatchProvider {
        ...
        public RequestDispatcher create(AbstractResourceMethod method) {
            ...
            if (method.getMethod().isAnnotationPresent(Authorized.class) || method.getDeclaringResource().getResourceClass().isAnnotationPresent(Authorized.class)) {
            ...
            if (method.getMethod().isAnnotationPresent(ExistingTenant.class) || method.getDeclaringResource().isAnnotationPresent(ExistingTenant.class)) {
