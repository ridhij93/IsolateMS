...
public class DBIAttachmentStoreServer extends DBIEntityStore implements AttachmentStore, Initializable {
    ...
    public Attachment create(Attachment attachment) throws EntityAlreadyExistsException, InvalidEntityException {
        ...
        if (!LoadedAttachment.class.isAssignableFrom(attachment.getClass())) {
            throw new InvalidEntityException("Cannot create an attachment which data is not loaded. Attachment must be" + " an instance of LoadedAttachment");
        }
        this.dao.begin();
        ...
        try {
            ...
            if (Strings.isNullOrEmpty(attachment.getExtension())) {
                ...
                if (guessedExtension.isPresent()) {
                    ...
                    throw new InvalidEntityException("No extension were set in attachment entity and we failed to " + "guess one");
                }
            }
    ...
    public static String getATTACHMENT_TABLE_NAME() {
        return ATTACHMENT_TABLE_NAME;
    }
