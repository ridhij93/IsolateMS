...
public class ImageOptionsProviderServer implements InjectableProvider<Context, Parameter>, Provider {
    ...
    private class ImageOptionsInjectable extends AbstractHttpContextInjectable<Optional<ImageOptions>> {
        ...
        public Optional<ImageOptions> getValue(HttpContext httpContext) {
            if (httpContext.getRequest().getQueryParameters().containsKey(WIDTH_OPTION) || httpContext.getRequest().getQueryParameters().containsKey(HEIGHT_OPTION)) {
                Optional<Integer> width = extractValue(httpContext.getRequest().getQueryParameters().get(WIDTH_OPTION));
                Optional<Integer> height = extractValue(httpContext.getRequest().getQueryParameters().get(HEIGHT_OPTION));
    ...
    public ComponentScope getScope() {
        return ComponentScope.getPerRequest();
    }
    ...
    private boolean isExtractable(Parameter param) {
        return param.getParameterClass().isAssignableFrom(Optional.class) && (param.getParameterType() instanceof ParameterizedType) && ((ParameterizedType) param.getParameterType()).getActualTypeArguments().length == 1 && ((ParameterizedType) param.getParameterType()).getActualTypeArguments()[0].equals(ImageOptions.class);
    }

    public static String getWIDTH_OPTION() {
        return WIDTH_OPTION;
    }

    public static String getHEIGHT_OPTION() {
