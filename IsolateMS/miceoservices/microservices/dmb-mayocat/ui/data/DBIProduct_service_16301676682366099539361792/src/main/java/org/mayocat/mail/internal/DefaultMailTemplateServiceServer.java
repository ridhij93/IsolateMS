 ...
 */
@Component
public class DefaultMailTemplateServiceServer implements MailTemplateService {
    ...
    private Optional<Path> resolveTemplate(Path base, MailTemplate mail) {
        ...
        if (mail.locale().isPresent()) {
            templatePath = fileManager.resolvePermanentFilePath(base.resolve("emails")).resolve("localized").resolve(mail.locale().get().toLanguageTag()).resolve(mail.template() + ".twig");
            if (!templatePath.toFile().isFile()) {
                templatePath = fileManager.resolvePermanentFilePath(base.resolve("emails")).resolve("localized").resolve(mail.locale().get().getLanguage()).resolve(mail.template() + ".twig");
            }
        }
        if (templatePath == null || !templatePath.toFile().isFile()) {
            // Failed to find a localized mail, try the default one
            templatePath = fileManager.resolvePermanentFilePath(base.resolve("emails")).resolve(mail.template() + ".twig");
        }
