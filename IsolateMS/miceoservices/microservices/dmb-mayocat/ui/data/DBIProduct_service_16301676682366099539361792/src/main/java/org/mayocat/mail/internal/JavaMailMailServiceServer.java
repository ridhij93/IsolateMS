 ...
 */
@Component
public class JavaMailMailServiceServer implements MailService {
    ...
    public void sendEmail(Mail mail) throws MailException {
        ...
        try {
            ...
            logger.debug("Sending email from [{}] to [{}] with subject [{}]", Joiner.on(", ").join(message.getFrom()), Joiner.on(", ").join(message.getAllRecipients()), message.getSubject());
