 ...
 */
public class DefaultWebContextServer implements WebContext {
    ...
    public DefaultWebContextServer(Tenant tenant, User user) {
        this.tenant = tenant;
        this.user = user;
    }
    ...
    public boolean isAlternativeLocale() {
        if (alternativeLocale == null) {
            throw new RuntimeException("Illegal attempt at accessing context alternative locale flag before it is initialized");
        }
    ...
    public Boolean getAlternativeLocale() {
        return alternativeLocale;
    }
